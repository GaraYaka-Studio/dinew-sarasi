...
export function StudentResultCard({
    ...
    if (status === 'idle') {
        return (
            <div className="flex h-full animate-in flex-col items-center justify-center space-y-4 rounded-xl border-2 border-dashed border-muted bg-muted/5 p-8 text-center duration-500 fade-in">
                ...
                <div>
                    <h3 className="playfair-font text-xl font-semibold text-foreground">
                        Ready to Scan
                    </h3>
                    <p className="max-w-xs text-sm text-muted-foreground">
                        Point the scanner at the QR code or manually search for
                        a student to begin.
                    </p>
                </div>
            </div>
    ...
    return (
        <div className="mx-auto flex h-full max-w-2xl animate-in flex-col items-center space-y-8 duration-500 slide-in-from-bottom-4">
            {/* 1. Profile Header */}
            <div className="flex flex-col items-center space-y-4">
                ...
                <div className="space-y-1 text-center">
                    ...
                    <p className="inline-block rounded-full bg-muted/30 px-3 py-1 font-mono text-lg text-muted-foreground">
                        {student.id}
                    </p>
                </div>
            </div>
                ...
                <CardContent className="flex items-center justify-between p-4 px-6">
                    ...
                    <div className="flex gap-2">
                            ...
                            <div
                                ...
                                className={cn(
                                    ...
                                    present
                                        ...
                                        : 'bg-red-200 ring-red-100 dark:bg-red-900/50 dark:ring-red-900'
                                )}
                        ...
                        {[...Array(2)].map((_, i) => (
                            <div
                                key={`future-${i}`}
                                className="h-4 w-4 rounded-full border-2 border-muted bg-transparent"
                            />
                        ))}
                    </div>
                </CardContent>
                    ...
                    <div className="flex w-full items-center justify-between">
                        ...
                        {student.arrears && student.arrears > 0 && (
                            <Button
                                size="sm"
                                variant="destructive"
                                className="md:text-md animate-pulse shadow-lg"
                            >
            ...
            {onMarkPresent && (
                <div className="flex w-full gap-4 pt-4">
                    <Button
                        variant="outline"
                        size="lg"
                        className="h-14 flex-1 border-2 text-lg hover:bg-muted/80"
                        onClick={onCancel}
                    >
                        Cancel
                    </Button>
                    <Button
                        size="lg"
                        className="h-14 flex-1 text-lg font-bold shadow-xl shadow-primary/20 transition-transform hover:scale-[1.02]"
