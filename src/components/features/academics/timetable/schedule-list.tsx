    ...
    if (sessions.length === 0) {
        return (
            <div className="flex h-[300px] w-full flex-col items-center justify-center rounded-lg border border-dashed text-center">
                ...
                <p className="text-muted-foreground text-sm">Add a session to get started.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6 pb-20">
            {sortedTimes.map((time) => (
                <div key={time} className="grid grid-cols-1 gap-4 md:grid-cols-[100px_1fr]">
                    {/* Time Header (Desktop: Left Column, Mobile: Top Header) */}
                    <div className="flex items-start pt-2 md:justify-end">
                         <div className="bg-muted text-muted-foreground sticky top-4 rounded-md px-2 py-1 text-xs font-bold uppercase tracking-wider md:bg-transparent md:text-sm md:font-semibold">
                            {time}
                        </div>
                    </div>
