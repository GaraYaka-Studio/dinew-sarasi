  ...
  const subjectCode = useMemo(() => {
    if (subjectName.trim()) {
      const code = subjectName
        ...
        .join('')
        .slice(0, 4);
      return code + '-' + (section ? section.toUpperCase().slice(0, 2) : 'GEN');
    }
    return '';
  }, [subjectName, section]);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        {selectedGrades.length === 0 && (
          <p className="text-xs text-red-500">Please select at least one grade</p>
        )}
      </div>
      ...
      <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
        ...
        <Button
          type="submit"
          disabled={!subjectName.trim() || selectedGrades.length === 0}
        >
          Save Subject
        </Button>
      </div>
