...
export const attendanceRecords = pgTable(
    ...
    (table) => [
        ...
        ).using('btree', table.studentId, table.sessionId, table.date),
        index('idx_daily_attendance_report').using(
            'btree',
            table.date,
            table.classId
        ),
...
export const teachers = pgTable(
    ...
    (table) => [unique('teachers_nic_unique').on(table.nic)]
);
...
export const enrollments = pgTable(
    ...
    (table) => [
        uniqueIndex('idx_unique_enrollment').using(
            'btree',
...
export const students = pgTable(
    ...
    (table) => [
        index('idx_students_phone').using(
            'btree',
            table.phone.asc().nullsLast().op('text_ops')
        ),
        index('idx_students_qr').using(
            'btree',
            table.qrCode.asc().nullsLast().op('text_ops')
        ),
    ]
);
...
export const studentFees = pgTable(
    ...
    (table) => [
        index('idx_fee_status_lookup').using(
            'btree',
            table.classId.asc().nullsLast().op('int4_ops'),
            table.year.asc().nullsLast().op('enum_ops'),
            table.monthIndex.asc().nullsLast().op('int4_ops'),
        ...
        uniqueIndex().using(
            ...
            table.monthIndex.asc().nullsLast().op('uuid_ops')
        ),
        foreignKey({
            columns: [table.studentId],
            foreignColumns: [students.id],
