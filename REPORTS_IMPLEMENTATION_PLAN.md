...
/* Typography */
--font-base: 14px (0.875rem) --font-sm: 12px (0.75rem) --font-lg: 16px (1rem)
...
export async function getTeacherPaymentsForMonth(
    ...
    const classIds = [
        ...new Set(payments.map((p) => p.classId).filter(Boolean)),
    ];
    ...
    for (const payment of payments) {
        ...
        const studentCount = payment.classId
            ? (countMap.get(payment.classId) ?? 0)
            : 0;
...
describe('getStudentPaymentsForMonth', () => {
    ...
    it('should include both monthly and admission fees', async () => {
        ...
        expect(
            monthlyPayments.length + admissionPayments.length
        ).toBeGreaterThan(0);
    });
